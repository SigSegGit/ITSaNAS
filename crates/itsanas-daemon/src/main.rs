...
async fn main() {
    let data_dir = std::env::var("ITSANAS_DATA_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|_| PathBuf::from("./itsanas-data"));
    ...
    let sync_dir = std::env::var("ITSANAS_SYNC_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|_| PathBuf::from("./itsanas-synced"));

    let state = Arc::new(
        AppState::open(data_dir.clone(), sync_dir.clone()).expect("failed to open vault"),
    );
